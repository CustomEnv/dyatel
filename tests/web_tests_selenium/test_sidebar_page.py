...
from tests.web_tests_selenium.conftest import sidebar_page_path


@pytest.fixture
def opened_sidebar_page(selenium_driver):  # New page (based on existing) with opened sidebar at page loading
    ...
    selenium_driver.get(f'file://{sidebar_shown_page_url}')
    yield SidebarPageSelenium()
...
def test_sidebar_page_navigation(sidebar_page):
    ...
    assert TabsPageSelenium().wait_page_loaded()
